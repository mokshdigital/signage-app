...
import { ChevronDown, ChevronUp, ChevronLeft, ChevronRight, Filter, X } from 'lucide-react';
...
export function MyTimesheetsTable({ userId }: MyTimesheetsTableProps) {
    ...
    const [workOrders, setWorkOrders] = useState<{ id: string; work_order_number: string }[]>([]);
    ...
    useEffect(() => {
        async function loadFilterOptions() {
            try {
                const [activities, locations, wos] = await Promise.all([
                ...
                setWorkOrders(wos.map((wo: any) => ({ id: wo.id, work_order_number: wo.work_order_number || 'N/A' })));
            } catch (err) {
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            const result = await timesheetsService.getMyDaysPaginated(
                userId,
                {
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between md:hidden">
                ...
                >
                    ...
                    Filters
                    {(filters.status.length > 0 || filters.activityTypeId || filters.locationChipId || filters.workOrderId) && (
                ...
                <div className="flex flex-wrap gap-3 items-end">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            onChange={(e) => {
                                setFilters(prev => ({ ...prev, workOrderId: e.target.value }));
                                setCurrentPage(1);
                            }}
                            className="px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                        >
